...
void scheduler(void) {
  ...
  for (;;) {
    ...
    long winningTicket = random_at_most(totalTickets);
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        ...
        passedTickets += p->tickets;

        // If we meet the processes that don't win the 'lottery', skip them
        if (passedTickets < winningTicket) {
            continue;
        }
